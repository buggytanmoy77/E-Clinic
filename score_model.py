...
Input_data = pd.read_csv("Final_E-clinic_dataset.csv")
...
def calculate_health_score(row):
    ...
    norm_deviation_dbp = abs(diastolic_bp - ideal_dbp) / ideal_dbp if not (70 <= diastolic_bp <= 90) else 0
    ...
    return (total_weighted_score / total_ideal_weights) * 100

Input_data['Health Score'] = Input_data.apply(calculate_health_score, axis=1)

# NOTE: also fixed original bug — norm_deviation_dbp used systolic_bp instead of diastolic_bp
...
RF_model.fit(train_data[features], train_data['Health Score'])

# Save BOTH model and scaler together — critical for correct inference
with open('health_score_model.pkl', 'wb') as f:
